      ...
      <header className="border-b">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={copyCode} className="hover-scale" aria-label="Copy room code">
      ...
      <main className="container mx-auto px-4 py-8 animate-fade-in">
        <section className="grid gap-6 md:grid-cols-3">
          <Card className="md:col-span-2">
            ...
            <CardContent>
              <p className="text-muted-foreground">Waiting for players to join… (placeholder list)</p>
              <ul className="mt-4 grid grid-cols-2 gap-3">
                {["You", "Player 2", "Player 3", "Player 4"].map((p) => (
                  <li key={p} className="rounded-md border p-3">{p}</li>
                ))}
              </ul>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              />
              <Button onClick={() => toast({ title: "Saved", description: `Name set to ${name || "Player"}` })}>
                Save
              </Button>
              <p className="text-sm text-muted-foreground">Invite code: <span className="font-mono">{roomId}</span></p>
            </CardContent>
          </Card>
        </section>
      </main>
